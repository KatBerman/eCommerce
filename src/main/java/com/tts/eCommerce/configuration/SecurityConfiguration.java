...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	/*
	 * Not entirely sure what else needs to be changed here...
	 * 
	 * @Override protected void configure(HttpSecurity http) throws Exception {
	 * http.authorizeRequests() .antMatchers("/console/**").permitAll()
	 * .antMatchers("/storefront/index").permitAll()
	 * .antMatchers("/storefront/product").permitAll()
	 * .antMatchers("/storefront/cart").permitAll()
	 * .antMatchers("/custom.js").permitAll()
	 * .antMatchers("/custom.css").permitAll()
	 * .antMatchers().hasAuthority("USER").anyRequest()
	 * .authenticated().and().csrf().disable().formLogin()
	 * .loginPage("/login").failureUrl("/login?error=true")
	 * .defaultSuccessUrl("/order") .usernameParameter("username")
